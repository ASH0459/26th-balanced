/**
  ...
  * @file       Dev_Custom.c
  * @brief      图传链路数据Dev层，包括0x302自定义控制器数据、0x304键鼠数据接收
...
/**
  * @brief          串口中断服务函数
  *                 将在HAL_UARTEx_RxEventCallback串口1的空闲中断回调函数中被调用
  * @param[in]      huart: 对应的串口句柄
  *                 Size : 数据长度
  *@retval          none
  */
void UART1_ISR_Handler(UART_HandleTypeDef *huart,uint16_t Size)
{ 
    if (huart->Instance == USART1)
    {
        if (Size <= CUSTOM_QUEUE_ITEM_SIZE || Size <= RC_QUEUE_ITEM_SIZE)
        {
            HAL_UARTEx_ReceiveToIdle_DMA(&huart1, _rx_buf_, CUSTOM_QUEUE_ITEM_SIZE); // 重新开启接收下一次串口中断数据
            ...
            memcpy(&custom_info_W, _rx_buf_, sizeof(Custom_Data_t)); // 将字节数组复制到结构体
            xQueueSendFromISR(xCustom_Queue, &custom_info_W, NULL);

            memcpy(&RC_info_W, _rx_buf_, sizeof(Custom_Data_t)); // 将字节数组复制到结构体
