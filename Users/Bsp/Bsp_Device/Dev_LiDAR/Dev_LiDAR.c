...
__attribute__((section(".ram_d2"), aligned(4))) uint8_t lidar_rx_buf[UART_RX_BUFFER_SIZE];
...
void UART1_Error_Handler(void)
{
    ...
    // {
    //     __HAL_UART_CLEAR_FLAG(&huart1, UART_CLEAR_OREF); // 清除错误标志
    //     volatile uint32_t tmpreg = huart1.Instance->RDR; // 丢弃废数据
    //     (void)tmpreg;
    // }
    //
    // // 不管什么错误，再次重新把它叫醒接收
    // HAL_UARTEx_ReceiveToIdle_DMA(&huart7, lidar_rx_buf, sizeof(lidar_rx_buf));
}
