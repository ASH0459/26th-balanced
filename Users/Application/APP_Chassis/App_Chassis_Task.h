...
// 关节最大最小力矩
#define  JOINT_MAX_TORQUE				60.0f
#define  JOINT_MIN_TORQUE				-60.0f

// 腿的最大最小长度
#define CHASSIS_LEG_MAX					0.3f
#define CHASSIS_LEG_MIN					0.1f
...
// 驱动轮 轮距的一半
#define DRIVE_WHEEL_PITCH				0.2263f
...
class Chassis_Move
{
    ...
    const Gimbal_Data *chassis_gimbal_data;

    Chassis_Mode_e chassis_mode;                  //底盘控制状态机
    Chassis_Mode_e last_chassis_mode;             //底盘上次控制状态机

    Chassis_Joint_Motor chassis_joint_measure[4]; //底盘关节电机数据
    Chassis_Wheel_Motor chassis_wheel_measure[2]; //底盘轮电机数据

    leg_control chassis_left_control;			  // 左腿控制类
    leg_control chassis_right_control;			  // 右腿控制类

    fp32 v_real;														// 实际速度
    fp32 v_filter;														// 滤波后的速度
    ...
    fp32 vaEstimateKF_F_VX[4];												// 配置状态转移矩阵 F（离散时间模型）
    fp32 vaEstimateKF_P_VX[4];												// 初始协方差 P
    fp32 vaEstimateKF_Q_VX[4];												// 配置过程噪声协方差 Q
    fp32 vaEstimateKF_R_VX[4];												// 配置测量噪声协方差 R
    fp32 vaEstimateKF_H_VX[4];												// 配置测量矩阵 H（直接测量两个状态）
    fp32 vaEstimateKF_F_theta[4];											// 配置状态转移矩阵 F（离散时间模型）
    fp32 vaEstimateKF_P_theta[4];											// 初始协方差 P
    fp32 vaEstimateKF_Q_theta[4];											// 配置过程噪声协方差 Q
    fp32 vaEstimateKF_R_theta[4];											// 配置测量噪声协方差 R
    fp32 vaEstimateKF_H_theta[4];											// 配置测量矩阵 H（直接测量两个状态）
