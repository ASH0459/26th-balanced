...
{
    static TickType_t tick_cnt = xTaskGetTickCount();

    // 自定义控制器
    if(pdPASS == xQueueReceive(VT_Moudle->custom_robot.xcustom_queue, custom_control.data, NULL))
    {
        detect_hook(VT_TOE);
    }

    // 图传链路键鼠
    if(pdPASS == xQueueReceive(VT_Moudle->remote_robot.xremote_queue, remote_control.data, NULL))
    {
        ...
        memcpy(&remote_control.mouse_x, &remote_control.data[0], sizeof(remote_control.mouse_x));
        memcpy(&remote_control.mouse_y, &remote_control.data[2], sizeof(remote_control.mouse_y));
        memcpy(&remote_control.mouse_z, &remote_control.data[4], sizeof(remote_control.mouse_z));
        memcpy(&remote_control.left_button_down, &remote_control.data[6], sizeof(remote_control.left_button_down));
        memcpy(&remote_control.right_button_down, &remote_control.data[7], sizeof(remote_control.right_button_down));
        memcpy(&remote_control.keyboard_value, &remote_control.data[8], sizeof(remote_control.keyboard_value));

        detect_hook(VT_TOE);
    }

    // 新图传键鼠+遥控
    if(pdPASS == xQueueReceive(VT_Moudle->vt_rc_robot.xrc_vt_queue, vt_rc_control.data, NULL))
    {
        //帧头 2bytes
        vt_rc_control.sof_1 = vt_rc_control.data[0];
        vt_rc_control.sof_2 = vt_rc_control.data[1];

        // 提取 8 字节的 bitfield 数据（data[2] ~ buf[9]）
        uint64_t bitfield = 0;
        for (int i = 0; i < 8; i++)
        {
            bitfield |= ((uint64_t)vt_rc_control.data[i + 2]) << (8 * i);
        }
        ...
        // 键鼠键位 9bytes
        vt_rc_control.mouse_x = (int16_t)(vt_rc_control.data[10] | (vt_rc_control.data[11] << 8));
        vt_rc_control.mouse_y = (int16_t)(vt_rc_control.data[12] | (vt_rc_control.data[13] << 8));
        vt_rc_control.mouse_z = (int16_t)(vt_rc_control.data[14] | (vt_rc_control.data[15] << 8));

        uint8_t mouse_byte = vt_rc_control.data[16];
        ...
        // 控制刷新频率，确保捕捉边沿
        if(xTaskGetTickCount() - tick_cnt > 20)
        {

            tick_cnt = xTaskGetTickCount();
        }
        vt_rc_control.key   = vt_rc_control.data[17] | (vt_rc_control.data[18] << 8);

        // CRC校验 2bytes
        vt_rc_control.crc16 = vt_rc_control.data[19] | (vt_rc_control.data[20] << 8);
